...
function createUnexpectedMap(expect) {
        ...
        .all([
          expect.promise(() => {
            if (expect.flags.exhaustively) {
              var nonOwnKeysWithDefinedValues = keys.filter(key => {
                return (
                  !Object.prototype.hasOwnProperty.call(subject, key) &&
                  typeof subject[key] !== 'undefined'
                );
              });
              var valueKeysWithDefinedValues = keys.filter(key => {
                var valueKey = valueIsMap ? value.get(key) : value[key];
                return typeof valueKey !== 'undefined';
              });
              var subjectKeysWithDefinedValues = subjectKeys.filter(key => {
                var subjectKey = subjectIsMap ? subject.get(key) : subject[key];
          ...
          return expect.promise.settle(promisesInFlight).then(() => {
            expect.fail({
              diff(output, diff, inspect, equal) {
                ...
                var uniqueKeys = subjectType
                  .uniqueKeys(subjectKeys, valueKeys)
                  .filter(key => {
                    var valueKey = valueIsMap ? value.get(key) : value[key];
                    var subjectHasKey = subjectIsMap
                      ? subject.has(key)
                      : key in subject;
  ...
  expect.addAssertion(
    ...
    (expect, subject, nextArg) => {
      ...
      var expected = subjectIsMap ? new Map() : {};
      var setExpected = subjectIsMap
        ? (key, value) => {
